...
class Arena implements Listener {
    ...
    public function joinToArena(Player $p) {
        if ($p->hasPermission("bg.access") || $p->isOp()) {
            ...
            $p->addTitle("§bWelcome To", "", 20, 40, 20);
            $p->addSubtitle("§eBattle§cgrounds","",20, 40, 20);
            $p->removeAllEffects();
            $vars = ['%1'];
            $replace = [$p->getName() ];
    ...
    public function openIngame($p) {
        if ($p instanceof Player) {
            ...
            $form->setContent("The game is currently running. Do you want to spectate them?\n\n\n");
    ...
    public function dropGuns($p, $block, $level) {
        switch (mt_rand(1, 4)) {
            ...
            case 4:
                ...
                $itemName = $item->setCustomName(TextFormat::AQUA . "CrossBow Ammo");
                $level->dropItem(new Vector3($block->x, $block->y, $block->z), $itemName);
            break;
        }
    }
    ...
    public function dropPotionKits($p, $block, $level) {
        switch (mt_rand(1, 7)) {
            ...
            case 4:
                ...
                $itemName = $item->setCustomName(TextFormat::AQUA . "Booster");
                $level->dropItem(new Vector3($block->x, $block->y, $block->z), $itemName);
            break;
        }
    }
